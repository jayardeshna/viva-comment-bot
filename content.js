...
// Track posts currently being processed (to prevent double-processing)
...
async function autoCommentOnPost(postEl) {
  ...
  // FIRST: Check if this element is already marked (most reliable check)
  ...
  if (processedPosts.has(postId) || currentlyProcessing.has(postId)) {
    return false
  }

  // Mark as processing IMMEDIATELY to prevent any duplicate attempts
  currentlyProcessing.add(postId)
  ...
  if (!postContainsKeyword(postEl)) {
    ...
    currentlyProcessing.delete(postId)
    return false
  }
...
function findAllPosts(container) {
  ...
  for (const el of allElements) {
    ...
    const hasCriticalKeyword = CRITICAL_KEYWORDS.some((kw) =>
